...
const SuccessPage = ({
  ...
  useEffect(() => {
    const updateOrderStatus = async () => {
      ...
      try {
        const result = await updatePedidoByClientId(client_id, 'completed')
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-4">
      ...
      <h1 className="text-2xl font-bold mt-2 mb-2">¡Pago Exitoso!</h1>
      <p className="text-lg text-center mb-4">
        Tu pago ha sido procesado correctamente. Pronto nos pondremos en contacto contigo para coordinar la instalación.
      </p>

      {updateStatus === 'loading' && <p className="text-sm text-blue-500 mb-4">Confirmando tu pedido...</p>}

      {updateStatus === 'success' && (
        <p className="text-sm text-green-500 mb-4">Tu pedido ha sido confirmado correctamente.</p>
      )}

      {updateStatus === 'error' && (
        <p className="text-sm text-red-500 mb-4">
          No pudimos confirmar tu pedido en nuestro sistema, pero tu pago fue exitoso. No te preocupes, nos pondremos en
